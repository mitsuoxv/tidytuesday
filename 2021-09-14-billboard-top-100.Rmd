...
top1_hits <- billboard %>% 
  filter(peak_position == 1) %>% 
  distinct(performer, song, song_id) %>% 
  left_join(top1_songs, by = "song_id") %>% 
...
p1 <- top1_hits %>% 
  ...
  geom_boxplot() +
  labs(x = NULL, y = "# of weeks staying at top 1",
       title = "By song",
...
top1_performers <- top1_hits %>% 
  filter(decade > 1950, decade < 2020) %>% 
  group_by(decade, performer) %>% 
  summarize(n_top1 = sum(n_top1)) %>% 
  group_by(decade) %>% 
  mutate(n_performers = n()) %>% 
  ungroup() %>% 
  mutate(decade = glue::glue("{decade}s\n({n_performers})"))
...
p2 <- top1_performers %>% 
  ggplot(aes(decade, n_top1)) +
  geom_boxplot() +
  geom_text_repel(aes(label = performer), size = 3, data = top1_performers %>% filter(n_top1 > 100)) +
  labs(x = NULL, y = NULL,
       title = "By performer",
