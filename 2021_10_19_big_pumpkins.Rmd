...
pumpkins <- pumpkins %>% 
  mutate(
    ...
    pollinator_father = if_else(pollinator_father %in% c("self", "SELF"), "Self", pollinator_father),
...
p <- great_pumpkins %>% 
  filter(!is.na(pollinator_father)) %>% 
  mutate(
    father_is_self = pollinator_father == "Self",
    pollinator = if_else(father_is_self, "Self", "Not self")
    ) %>% 
  ...
  labs(x = "Weight (pounds)", y = "Pollinator",
       title = "Self pollinated tend to weight less in Great Pumpkins",
       subtitle = "( ) denotes # of observations",
       caption = "BigPumpkins.com")
