...
big_dave <- tt$big_dave %>% 
  ...
  mutate(
    ...
    clue_temp = clue %>% 
      str_match("\\(([\\d,-’\\s]+)\\)"),
    clue_num = clue_temp[, 2] %>% 
      str_replace_all("-", ","),
    puzzle_name_temp = puzzle_name %>% 
      str_to_lower() %>% 
      str_match("^(\\D*)([\\d|,]*)$"),
    puzzle_name_source = puzzle_name_temp[, 2] %>% 
      str_trim() %>% 
      str_remove(" –|-| no$"),
    puzzle_name_id = puzzle_name_temp[, 3]
  ) %>% 
  select(-clue_temp, -puzzle_name_temp)
...
p <- plot_data %>% 
  ...
  labs(x = "Puzzle Name", y = "# of letters of single-word answers",
